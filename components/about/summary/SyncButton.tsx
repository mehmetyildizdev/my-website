...
export function SyncButton({
  ...
  const getTabColorAngle = (color: string) => {
    switch (color) {
      case "ruby": return 0;
      case "amethyst": return 72;
      case "sapphire": return 144;
      case "emerald": return 216;
      case "topaz": return 288;
      default: return 0;
    }
  };
  ...
  return (
    ...
    >
      ...
      <m.div style={{ x: inverseX }} className="absolute inset-0 flex justify-start items-center pl-2 z-10 pointer-events-none">
        <p className={cn("text-platinum light:text-pearl text-xl flex justify-start items-center gap-2 drop-shadow-[0_2px_3px_rgba(0,0,0,0.9)] transition-opacity duration-300 whitespace-nowrap", hideContent ? "opacity-0" : "opacity-100")}>
