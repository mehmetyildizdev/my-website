...
export const embedBlockType = defineType({
  ...
  fields: [
    ...
    },
    {
      ...
      type: "url",
      description:
        "Paste the embed URL. For YouTube: use the /embed/ form, e.g. https://www.youtube.com/embed/VIDEO_ID",
    ...
    },
    {
      ...
      rows: 10,
      description:
        "Paste a raw HTML snippet (canvas, WebGL animations, etc.). Runs in a sandboxed iframe — no access to the parent page.",
  ...
  preview: {
    ...
    prepare({ embedType, embedUrl, caption, alt }) {
      const label =
        caption || alt || (embedType === "url" ? embedUrl : "Custom HTML Code");
      return {
        title: label ?? "Embed",
