...
export function HeroImageContent({ kind, url, htmlCode, alt, themeBg, priority = false }: HeroImageContentProps) {
  ...
  return (
    <div className="relative h-full w-full overflow-hidden rounded-xl">
      {isLoading && <Skeleton className={cn('absolute inset-0 z-10 h-full w-full rounded-none opacity-40', themeBg)} />}
      ...
      {kind === 'image' && url ? (
        ...
        <iframe
          ...
          scrolling="no"
          onLoad={() => setIsLoading(false)}
          className={cn('h-full w-full border-none block transition-opacity duration-500', isLoading ? 'opacity-0' : 'opacity-100')}
          style={{
            width: '100%',
            height: '100%',
          }}
