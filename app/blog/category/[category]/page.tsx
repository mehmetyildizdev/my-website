...
export default async function CategoryPage({ params }: Props) {
  ...
  const categoryPosts = allPosts.filter((post) => post.categories?.some((cat) => cat.title.toLowerCase() === decodedCategory));
  ...
  return (
    <div className="flex flex-col gap-12">
      ...
      <header className="flex flex-col gap-4 text-left">
        <p className="text-sm font-bold uppercase tracking-[0.3em] text-sapphire drop-shadow-sm">Category</p>
        <h1 className="text-4xl font-black tracking-tight text-gold md:text-5xl text-shadow-lg">{titleCategory}</h1>
