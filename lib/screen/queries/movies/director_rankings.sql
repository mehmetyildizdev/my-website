...
SELECT
  ...
  (SELECT m2.title FROM watch_history wh2
   ...
   WHERE wh2.media_type = 'movie' AND wh2.my_rating IS NOT NULL
   ORDER BY wh2.my_rating DESC LIMIT 1) AS best_movie,
  (SELECT MAX(wh2.my_rating) FROM watch_history wh2
   JOIN movie_crew mc2 ON mc2.movie_tmdb_id = wh2.tmdb_id AND mc2.person_tmdb_id = p.tmdb_id AND mc2.job = 'Director'
   WHERE wh2.media_type = 'movie' AND wh2.my_rating IS NOT NULL) AS best_rating
