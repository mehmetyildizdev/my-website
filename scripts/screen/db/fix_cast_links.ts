 ...
 *
 * FLAGS:
 *   --shows  / --shows-only   : Only process TV shows (skip movies)
 *   --movies / --movies-only  : Only process Movies (skip TV shows)
 *   --all                    : Force re-link ALL media (even if cast already exists)
 *
 * EXAMPLES:
 *   # Force re-sync cast_order for ALL TV shows:
 *   pnpm tsx -r dotenv/config scripts/screen/db/fix_cast_links.ts --shows --all
 */
...
const moviesOnly = args.includes('--movies') || args.includes('--movies-only');
const showsOnly = args.includes('--shows') || args.includes('--shows-only');

const runMovies = !showsOnly;
const runShows = !moviesOnly;
...
async function main() {
  ...
  try {
    const targetDesc = runMovies && runShows ? 'MOVIES & TV SHOWS' : runMovies ? 'MOVIES ONLY' : 'TV SHOWS ONLY';
    console.log('\n============================================================');
    console.log('🎬 [Fix Cast Links] High-Speed Cast & Crew Repair Script');
    console.log(`📌 Target: ${targetDesc}`);
    console.log(`📌 Mode:   ${forceAll ? 'FORCE ALL (--all)' : 'AUTO (Only items missing cast links)'}`);
    console.log('============================================================\n');
    ...
    // ──────────────────────────────────────────────────────────────────────────
    if (!runMovies) {
      console.log('🎬 PHASE 1: Skipping Movies (--shows flag active).\n');
    } else {
    ...
    // ──────────────────────────────────────────────────────────────────────────
    if (!runShows) {
      console.log('📺 PHASE 2: Skipping TV Shows (--movies flag active).\n');
    } else {
    ...
    if (runMovies) console.log(`  • Movies Processed:       ${moviesProcessed}/${moviesCount}`);
    if (runShows)  console.log(`  • TV Shows Processed:     ${showsProcessed}/${showsCount}`);
