...
const GEM_CYCLE = ['var(--sapphire)', 'var(--ruby)', 'var(--emerald)', 'var(--amethyst)', 'var(--topaz)'];
  ...
  return (
    <Card className="bg-pearl/30 border-border/15 shadow-2xl backdrop-blur-md">
      <CardHeader className="pb-2">
        <CardTitle className="text-lg font-bold tracking-tight text-accent">Network Timeline</CardTitle>
          ...
          <div className="flex" style={{ minWidth: labelWidth + dataWidth }}>
            ...
            <div className="sticky left-0 z-10 bg-background/80 backdrop-blur-sm shrink-0" style={{ width: labelWidth }}>
              ...
              {networks.map(([name], networkIdx) => {
                ...
                return (
                  <div key={name} className="flex items-center gap-1.5 pr-2" style={{ height: ROW_HEIGHT }}>
            ...
            <div className="flex-1" style={{ minWidth: dataWidth }}>
              ...
              {networks.map(([name, { years: yearMap }], networkIdx) => {
                const color = getNetworkColor(name, networkIdx);
                return (
                  <div key={name} className="flex items-center hover:bg-pearl/20 rounded transition-colors" style={{ height: ROW_HEIGHT }}>
                    {years.map((y) => {
                      const entry = yearMap.get(y);
                      if (!entry) {
                        return (
                          <div key={y} style={{ width: yearColWidth }} className="shrink-0 flex justify-center items-center">
                            {EMPTY_CIRCLE_COLOR !== 'transparent' && (
                              <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: EMPTY_CIRCLE_COLOR }} />
                            )}
                          </div>
                        );
                      }
                      ...
                      return (
                          ...
                          content={
                            <div className="flex flex-col gap-0.5 min-w-[160px]">
                              ...
                              <div className="flex justify-between gap-4 text-[11px] mt-1">
                                ...
                                <span className="text-platinum font-semibold tabular-nums">{entry.count}</span>
                              </div>
                              <div className="flex justify-between gap-4 text-[11px]">
                                <span className="text-quicksilver">Avg Rating</span>
                                <span className="text-emerald tabular-nums">{entry.rating.toFixed(1)}</span>
                              </div>
                              {entry.shows.length > 0 && (
                                <div className="mt-1 pt-1 border-t border-border/20 space-y-0.5">
                                  ...
                                  {entry.shows.length > 8 && (
                                    <p className="text-[10px] text-muted-foreground">+{entry.shows.length - 8} more</p>
                                  )}
                                </div>
                              )}
                            </div>
                        ...
                        >
                          <div style={{ width: yearColWidth }} className="shrink-0 flex justify-center items-center">
                            <div
